            ...
            {step < 4 && (
                <div style={{ marginBottom: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem' }}>
                        {[1, 2, 3].map(i => (
                            <div key={i} style={{ flex: 1, height: '6px', borderRadius: '3px', background: i <= step ? 'var(--danger)' : 'var(--border-color)', transition: 'background 0.3s ease' }} />
                        ))}
                    </div>
                </div>
            )}
                ...
                <AnimatePresence mode="wait">
                    {step === 1 && (
                        <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                            <h2 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: 'var(--trust-blue)' }}>Step 1: Select Suspected Medicine</h2>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                {myMedicines.map(med => (
                                    <div 
                                        ...
                                        style={{ 
                                            ...
                                            border: selectedMed === med.id ? '2px solid var(--primary-color)' : '1px solid var(--border-color)',
                                            background: selectedMed === med.id ? 'rgba(27, 79, 114, 0.05)' : 'var(--bg-card)'
                                        }}
                                    >
                                        <div style={{ background: 'rgba(27, 79, 114, 0.1)', color: 'var(--primary-color)', padding: '0.75rem', borderRadius: '50%' }}>
                                        ...
                                        <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: '2px solid var(--border-color)', background: selectedMed === med.id ? 'var(--primary-color)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                ...
                                <input type="text" placeholder="Search other medicine..." style={{ padding: '1rem', borderRadius: '1rem', border: '1px solid var(--border-color)', background: 'var(--bg-input)' }} />
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 2 && (
                        <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                            <h2 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: 'var(--trust-blue)' }}>Step 2: Describe Reaction</h2>
                            <textarea 
                                value={reaction}
                                onChange={e => setReaction(e.target.value)}
                                placeholder="What symptoms did you experience after taking this medicine?"
                                rows={6}
                                style={{ width: '100%', padding: '1rem', borderRadius: '1rem', border: '1px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-light)', fontSize: '1rem', resize: 'vertical' }}
                            />
                            
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
                                <button className="glass-panel" style={{ flex: 1, padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', border: '1px dashed var(--border-color)', cursor: 'pointer', background: 'transparent' }}>
                    ...
                    {step === 3 && (
                        <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                            <h2 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: 'var(--trust-blue)' }}>Step 3: Severity Scale</h2>
                            <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>How severe was the adverse reaction?</p>
                            
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <div onClick={() => setSeverity('mild')} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', cursor: 'pointer', border: severity === 'mild' ? '2px solid var(--success)' : '1px solid var(--border-color)' }}>
                                ...
                                <div onClick={() => setSeverity('moderate')} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', cursor: 'pointer', border: severity === 'moderate' ? '2px solid #F39C12' : '1px solid var(--border-color)' }}>
                                ...
                                <div onClick={() => setSeverity('severe')} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', cursor: 'pointer', border: severity === 'severe' ? '2px solid var(--danger)' : '1px solid var(--border-color)' }}>
                    ...
                    {step === 4 && (
                        <motion.div key="step4" variants={stepVariants} initial="hidden" animate="visible" style={{ textAlign: 'center', paddingTop: '4rem' }}>
                            ...
                            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', padding: '1.5rem', borderRadius: '1rem', display: 'inline-block' }}>
                                <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reference ID</div>
                                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--trust-blue)', marginTop: '0.5rem' }}>ADR-2026-8892</div>
                            </div>
                            <div style={{ marginTop: '3rem' }}>
                                <button className="btn-primary" onClick={() => setStep(1)} style={{ width: 'auto', display: 'inline-flex' }}>Submit Another Report</button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
