...
const Alerts = () => {
    ...
    useEffect(() => {
        const fetchAlerts = async () => {
            try {
                const [recallsRes, expiryRes] = await Promise.all([
    ...
    return (
        <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'grid', gap: '1.5rem' }}>
            <div>
                <h1 style={{ fontSize: '1.75rem', margin: 0, color: 'var(--primary-color)' }}>Recall & Expiry Alerts</h1>
                <p style={{ color: 'var(--text-muted)', margin: '0.35rem 0 0' }}>Active recalls and near-expiry stock in your inventory.</p>
            </div>
