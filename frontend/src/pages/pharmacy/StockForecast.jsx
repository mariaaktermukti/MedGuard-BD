...
const StockForecast = () => {
    ...
    return (
        <div style={{ maxWidth: '760px', margin: '0 auto', display: 'grid', gap: '1.5rem' }}>
            <div>
                <h1 style={{ fontSize: '1.75rem', margin: 0, color: 'var(--primary-color)' }}>AI Stock Forecasting</h1>
            ...
            <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center' }}>
                <button type="button" className="btn-primary" onClick={generateForecast} disabled={loading} style={{ width: 'auto', padding: '0.75rem 1.5rem', margin: '0 auto' }}>
            ...
            {forecast && (
                <div className="glass-panel" style={{ padding: '2rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem' }}>
                        <ChartLineUp size={22} color="var(--primary-color)" />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1.5rem', alignItems: 'center' }}>
                        <div>
                            <div style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--primary-color)' }}>{forecast.predicted_demand}</div>
                            <div style={{ color: 'var(--text-muted)' }}>units predicted demand — {forecast.region}</div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <div style={{ width: '140px', height: '140px', borderRadius: '50%', background: `conic-gradient(var(--primary-color) 0% ${confidencePercent}%, rgba(148, 163, 184, 0.16) ${confidencePercent}% 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <div style={{ width: '96px', height: '96px', borderRadius: '50%', background: 'var(--bg-card)', boxShadow: 'inset 0 0 0 1px var(--border-color)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
