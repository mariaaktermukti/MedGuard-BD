...
const RouteOptimization = () => {
    ...
    return (
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gap: '1.5rem' }}>
            <div>
                <h1 style={{ fontSize: '1.75rem', margin: 0, color: 'var(--primary-color)' }}>Route Optimization</h1>
                ...
                {loading ? (
                    ...
                    <div style={{ display: 'grid', gap: '1rem' }}>
                        {suggestions.map((group) => (
                            <div key={group.area} style={{ padding: '1.25rem', borderRadius: '0.85rem', border: '1px solid var(--border-color)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 700 }}>
                                        <MapPin size={18} color="var(--primary-color)" /> {group.area}
                                    </div>
